...
//  Serial.println(buttonState); //print the value of buttonState (1 or 0 because it's a boolean)
  ...
  { //if the statement is true, do what is in these curly brackets
    ...
    counter = counter % 8; //modulo operator, counter can only count to 3
...
//    //if the counter is at 0, turn on LED 1 (pin 5)
//    if (counter == 0)
//    {
//      digitalWrite(5, HIGH);
//    }
...
//    }
//    
//    //if the counter is at 1, turn on LED 2 (pin 8)
//    if (counter == 1)
//    {
//      digitalWrite(8, HIGH);
//    }
...
//    }
//
//    //if the counter is at 2, turn on LED 3 (pin 11)
//    if (counter == 2)
//    {
...
//    }
    ...
    Serial.println(bitRead(counter, 0));  //print just bit 0 of 'counter'
    Serial.print("bitRead(counter, 1): ");
    Serial.println(bitRead(counter, 1));  //print just bit 1 of 'counter'
    Serial.print("bitRead(counter, 2): ");
    Serial.println(bitRead(counter, 2));  //print just bit 2 of 'counter'
    
    if(bitRead(counter, 0) == 1) //if bit 0 of the 'counter' is 1, turn LED 1 on
    {
    ...
    if(bitRead(counter, 1) == 1) //if bit 1 of the 'counter' is 1, turn LED 2 on
    {
    ...
    if(bitRead(counter, 2) == 1) //if bit 2 of the 'counter' is 1, turn LED 3 on
    {
      digitalWrite(11, HIGH); //LED 3 on
    }
    else
    {
      digitalWrite(11, LOW); //LED 3 off
    }
  }
}
